...
async function apiFetch(path, options = {}) {
  ...
  const res = await fetch(url, {
    ...
    headers: {
      ...(options.headers || {}),
      // se você usa algum header de permissão, adicione aqui:
      // 'X-User-Type': '1',
    }
  });

  // tenta ler JSON mesmo quando dá erro (pra mostrar msg do back)
  const text = await res.text();
  const data = text ? (() => { try { return JSON.parse(text); } catch { return text; } })() : null;

  if (!res.ok) {
    const msg = (data && data.message) ? data.message : (typeof data === 'string' ? data : 'Erro na API');
    throw new Error(msg);
  }
...
// POST /api/funcionarios/{id}/documentos?pastaId=123
async function apiUploadDocumento(funcionarioId, file, pastaId = null) {
  const fd = new FormData();
  fd.append('file', file); // IMPORTANTE: nome "file" (igual parâmetro do controller)

  const qs = pastaId ? `?pastaId=${encodeURIComponent(pastaId)}` : '';
  return apiFetch(`/api/funcionarios/${funcionarioId}/documentos${qs}`, {
    method: 'POST',
    body: fd
  ...
  function getFuncionariosLS() {
    ...
    arr.forEach((f, i) => {
      ...
      // NOVO – cursos e tipo de contrato
      if (typeof f.cursos === 'undefined')       f.cursos = '';
      if (typeof f.tipoContrato === 'undefined') f.tipoContrato = '';
    });
    localStorage.setItem(LS_KEYS.FUNCS, JSON.stringify(arr));
    return arr;
  }
  ...
  // deixa o gerador de avatar disponível globalmente (pro crachá usar igual)
    window.defaultAvatarDataURL = defaultAvatarDataURL;
  ...
  // ✅ (2B) Upload de FOTO do funcionário (multipart/form-data)
  // Requer um endpoint no back: POST /api/funcionarios/{id}/foto
      ...
      list
        .map((f) => {
          ...
          <div class="emp-menu-dropdown" data-menu="${f.id}" role="menu">
            ...
            <button data-action="cracha" data-id="${f.id}">
          ...
          <div class="emp-card ${inactiveClass}" data-id="${f.id}">
            ${menu}
            ${offBadge}
            ${photoOrFallback(resolveFotoSrc(f))}
            <div>
...
function openFuncionario(empId) {
  ...
  if (funcModalAvatar) {
    ...
    funcModalAvatar.src = src || (typeof defaultAvatarDataURL === 'function' ? defaultAvatarDataURL() : (window.defaultAvatarDataURL ? window.defaultAvatarDataURL() : ''));
  ...
  try {
    ...
    if (cadFoto?.files?.[0]) {
      try {
        ...
        // aceite qualquer nome que você retornar no back
        fotoUrl = resp.fotoUrl || resp.url || resp.path || null;
      } catch (err) {
        console.error(err);
