...
(() => {
  ...
  function canSeeOwnerRole(ownerRole, viewerRole) {
    ...
    if (v === 2) return o === 1 || o === 2 || o === 3; // ✅ Gestão vê Admin + Gestão + Segurança
  ...
  async function apiFetch(path, options = {}) {
    ...
    // ✅ Se você usa login por cookie/sessão, precisa enviar cookies no fetch
    const opt = { ...options };
    if (opt.credentials == null) opt.credentials = 'include';
  ...
  async function apiGetViewerRole() {
    ...
    // ✅ seu endpoint /api/usuarios/me costuma devolver "typeUser"
    const role = Number(
      data?.typeUser ?? data?.TypeUser ??
    ...
    function renderFuncionarios(q = '') {
      ...
      empGrid.innerHTML =
        (list.map((f) => {
          ...
          const isST = (STATE.viewerRole === 3);

          const actionsHtml = isST
            ? '' // ✅ ST: dropdown sem nenhuma opção
            : `
                ${
                  canManageStatus
                    ? (ativo
...
if (modalFuncionario) modalFuncionario.style.display = 'flex';
    ...
    empGrid?.addEventListener('click', (e) => {
      ...
      if (act === 'assinatura') {
        if (STATE.viewerRole === 3) {
          notify('Perfil Segurança do Trabalho não tem acesso a Gerar assinatura.', 'warn');
          closeAllEmpMenus();
          return;
        }
        window.gerarAssinaturaFuncionario?.(id);
      }
