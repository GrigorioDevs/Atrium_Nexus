...
// ✅ Protegido contra carregamento DUPLICADO (intervalo/listeners duplicados)

(function () {
  "use strict";

  // =========================================================
  // GUARD: evita inicializar duas vezes se o loader injetar o script novamente
  // =========================================================
  if (window.__ATRIUM_ALERTS_INITED__ === true) {
    // console.warn("alert.js já inicializado — ignorando duplicado");
    return;
  }
  window.__ATRIUM_ALERTS_INITED__ = true;
  ...
  async function init() {
    ...
    // 0) CONFIG
    // =========================================================
    // Se você usa Nginx com /api no mesmo domínio, pode deixar vazio.
    // Se quiser forçar, no HTML: window.API_BASE = "https://hml.atrium.tech";
    const API_BASE = String(window.API_BASE || "").replace(/\/+$/, "");
    const ALERTS_API_URL = `${API_BASE}/api/lembretes/docs-importantes`;
    ...
    const modalCadastro = document.getElementById("modalFuncionario");
    ...
    // =========================================================
    // 3) HELPERS UI (aria-hidden + foco + scroll lock robusto)
    // =========================================================
    ...
    let openCount = 0; // ✅ robusto: não depende de classes específicas no HTML

    function openModal(modal) {
      if (!modal) return;

      // só incrementa se realmente estava fechado
      const wasOpen = modal.classList.contains("open");
      if (!wasOpen) openCount++;
    ...
    function closeModal(modal) {
      ...
      if (wasOpen) openCount = Math.max(0, openCount - 1);
      if (openCount === 0) lockBodyScroll(false);
    ...
    // ✅ evita duplicar o ESC se alguém recarregar script
    if (!document.documentElement.dataset.alertsEscBound) {
      document.documentElement.dataset.alertsEscBound = "1";
    ...
    if (btnNovoLembrete) {
      bindOnce(btnNovoLembrete, "novoBound", () => {
        ...
        setTimeout(() => {
          const funcRoot =
            document.querySelector(".menu-func-root.has-submenu") ||
            document.querySelector(".menu-func-root");

          if (funcRoot) {
            funcRoot.setAttribute("aria-expanded", "true");
            funcRoot.classList.add("open");
          }

          if (typeof window.activateTab === "function") {
            window.activateTab("tabFuncGestao");
          } else {
            const li = document.querySelector('#menu [data-tab="tabFuncGestao"]');
            if (li && typeof li.click === "function") {
              li.click();
            } else {
              document.querySelectorAll("main .tab").forEach((t) => t.classList.remove("active"));
              const tab = document.getElementById("tabFuncGestao");
              if (tab) tab.classList.add("active");
            }
          }

          const hdr = document.getElementById("hdrFuncGestao");
    ...
    // ✅ evita interval duplicado se alguém tentar reiniciar o script
    if (window.__ATRIUM_ALERTS_INTERVAL__) {
      clearInterval(window.__ATRIUM_ALERTS_INTERVAL__);
    }
    window.__ATRIUM_ALERTS_INTERVAL__ = setInterval(() => verificarVencimentos({ silent: true }), ONE_HOUR);
